...
impl<T: StakingConfig> StakingPallet<T> {
    ...
    pub fn stake(&mut self, who: T::AccountId, amount: T::Balance) -> Result<(), &'static str> {
        let free_balance = *self.free_balances.get(&who).unwrap_or(&T::Balance::zero());
        if let Some(new_balance) = free_balance.checked_sub(&amount) {
            self.free_balances.insert(who.clone(), new_balance); // Clone who
            let staked_balance = *self.staked_balances.get(&who).unwrap_or(&T::Balance::zero());
    ...
    pub fn unstake(&mut self, who: T::AccountId, amount: T::Balance) -> Result<(), &'static str> {
        let staked_balance = *self.staked_balances.get(&who).unwrap_or(&T::Balance::zero());
        if let Some(new_staked) = staked_balance.checked_sub(&amount) {
            self.staked_balances.insert(who.clone(), new_staked); // Clone who
            let free_balance = *self.free_balances.get(&who).unwrap_or(&T::Balance::zero());
